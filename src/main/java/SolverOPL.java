...
//        toNetwork();
    ...
    private pRouter addRouter(int index) {
        ...
        pRouter pRouter;
        if ((pRouter = (pRouter) ParserOPL.getRouterByName(network, name)) != null)
            return pRouter;
        int Bw = router.getIntValue("Bw");
        int Mw = router.getIntValue("Mw");
        int Lw = router.getIntValue("Lw");
        pRouter = new pRouter(name, Bw, Mw, Lw);
    ...
    private vRouter addVRouter(int req, int index) throws IloException {
        ...
        if (network.getRequests().size() >= req && (vRouter = (vRouter) ParserOPL.getRouterByName(network.getRequests().get(req - 1), name)) != null)
            return vRouter;
    ...
    public void solve(int i) throws IloException {
...
//            double aux = RC.getOplModel().getElement("aux").asNum();
